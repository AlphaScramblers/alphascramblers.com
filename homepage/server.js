...
// Middleware
app.use(cors()); // Allow all origins; you can restrict to your domain
...
function generateBarChart(science, commerce, humanities) {
  ...
  qc.setConfig({
    ...
    data: {
      ...
      datasets: [{ label: "Scores", data: [science, commerce, humanities], backgroundColor: ["#4caf50", "#2196f3", "#ff9800"] }],
    },
    options: {
      plugins: { title: { display: true, text: "Stream Scores", font: { size: 18 } } },
...
function generateAboutText(science, commerce, humanities, maxStream) {
  return `The student has shown interest in multiple streams. Science: ${science}, Commerce: ${commerce}, Humanities: ${humanities}. Recommended Stream: ${maxStream}. Demonstrates potential in analytical, numerical, and creative thinking.`;
}
...
app.post("/generate-report", async (req, res) => {
  ...
  try {
    ...
    page.drawText(`About:\n${generateAboutText(scienceScore, commerceScore, humanitiesScore, maxStream)}`, { x: 50, y: height - 200, size: 12, font, lineHeight: 16, maxWidth: 500 });

    // Charts
    const barChartBytes = await fetch(generateBarChart(scienceScore, commerceScore, humanitiesScore)).then(r => r.arrayBuffer());
    const barChartImage = await pdfDoc.embedPng(barChartBytes);
    page.drawImage(barChartImage, { x: 50, y: 300, width: 250, height: 200 });

    const pieChartBytes = await fetch(generatePieChart(scienceScore, commerceScore, humanitiesScore)).then(r => r.arrayBuffer());
